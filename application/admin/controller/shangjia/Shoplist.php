<?php
    ...
    use think\Request;
    use think\Url;
    use app\admin\model\user\UserShop as UserModel;
    ...
    class Shoplist extends AuthController
    {
        ...
        public function index(){
            //var_dump(SystemUserLevel::where(['is_show'=>1,'is_del'=>0])->field(['id','name'])->select());exit;
        ...
        public function get_user_list(){
            ...
            if (input ('get.status')==='0'||input ('get.status')==='1'){
                $wheres['a.status']=['eq',input ('get.status')];
            }else{
                $wheres['a.status']=['in','0,1'];
                
            }
            //return json(['code'=>0,'where'=>$where]);
            
            $data=db('user_shop')->alias('a')->where ($wheres)->join ('eb_user_des b','a.uid=b.uid')->field('a.*,b.name')->page($page,20)->select ();
            
            foreach ($data as $k=>$v){
                if ($v['user_type']==1){
                    $data[$k]['user_type']='商家';
                }else{
                    $data[$k]['user_type']='普通用户';
                }
                if ($v['shifou']==1){
                    $data[$k]['shifou']='是';
                }else{
            ...
            $count=db('user_shop')->alias('a')->where ($wheres)->join ('eb_user_des b','a.uid=b.uid')->field('a.*,b.name')->count ();
            return Json::successlayui(['count'=>$count,'data'=>$data]);
        }
        /**
         * 编辑模板消息
         * @param $id
